...
const EntityListProvider = ({
  ...
  const [backendEntities, setBackendEntities] = React.useState([]);
  const [backendFilters, setBackendFilters] = React.useState(reduceCatalogFilters(lodash.compact(Object.values(filters))));
  React.useEffect(() => {
    const newBackendFilters = reduceCatalogFilters(lodash.compact(Object.values(filters)));
    if (!lodash.isEqual(newBackendFilters, backendFilters)) {
  ...
  reactUse.useDebounce(refresh, 10, [backendFilters]);
  React.useEffect(() => {
    const resolvedEntities = (backendEntities != null ? backendEntities : []).filter(reduceEntityFilters(lodash.compact(Object.values(filters))));
